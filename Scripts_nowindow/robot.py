...
class BeaconRobot:
	...
	def scan_environment(self, time, map:Map):
		...
		lidar_data = self.lidar.scan_environment(time, map)
		...
		pos_lidar = self.correct_pos_with_lidar(points)
	...
	def correct_pos_with_lidar(self, points:list):
	...
	def compute_robot_collision(self, map:Map, ):
			...
			for idys in range(idy-1, idy+2):
				...
				if map_subdiv != []:
					...
					for wall in walls:
						...
						ortho_point = wall_orthogonal_point(self.pos.to_tuple(), wall, )
